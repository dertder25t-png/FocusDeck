      ...
      <div className="relative mr-2">
        ...
        {typeof window !== 'undefined' && localStorage.getItem('focusdeck_access_token') ? (
          <>
            ...
            {showUserMenu && (
              <div className="absolute left-0 bottom-full mb-2 w-56 bg-surface dark:bg-gray-800 border-2 border-ink dark:border-gray-600 rounded-xl shadow-hard z-50 overflow-hidden">
                <div className="px-4 py-3 border-b-2 border-ink dark:border-gray-600 bg-subtle dark:bg-gray-700 flex items-center gap-2">
                  <i className="fa-solid fa-id-badge text-accent-blue"></i>
                  <div className="text-xs font-mono truncate dark:text-gray-200" title={userId}>{userId}</div>
                </div>
                <div className="flex flex-col py-2">
                  <button
                    onClick={() => { setShowUserMenu(false); launchApp('win-account-settings'); }}
                    className="text-left px-4 py-2 text-xs font-semibold hover:bg-white/70 dark:hover:bg-gray-700 transition flex items-center gap-2 dark:text-gray-200"
                  >
                  ...
                  <button
                    onClick={() => { setShowUserMenu(false); window.location.href = '/integrations'; }}
                    className="text-left px-4 py-2 text-xs font-semibold hover:bg-white/70 dark:hover:bg-gray-700 transition flex items-center gap-2 dark:text-gray-200"
                  >
                    <i className="fa-solid fa-key"></i> API Keys & Integrations
                  </button>
          ...
          <button
            ...
            className="h-10 px-4 rounded-lg flex items-center gap-2 bg-accent-blue text-white text-sm font-bold shadow-hard hover:bg-blue-600 transition active:scale-[0.97]"
          >
