...
import { apiFetch, dashboard } from '../../../services/api';
...
export const HabitTrackerWidget: React.FC = () => {
  ...
  const handleToggle = async (id: string) => {
    try {
      await dashboard.toggleHabit(id);
      refetch();
    } catch (e) {
      console.error(e);
    }
  ...
  return (
    <div className="flex flex-col h-full bg-surface p-4 justify-center">
      ...
      {habits.length === 0 && <div className="text-xs text-gray-400 italic text-center">No habits added</div>}
      <div className="flex justify-between px-2 gap-2 overflow-x-auto custom-scrollbar">
        {habits.map(habit => (
          <button
            key={habit.id}
            onClick={() => handleToggle(habit.id)}
            className={`w-10 h-10 shrink-0 rounded-full flex items-center justify-center transition-all ${habit.isCompleted ? 'bg-green-500 text-white scale-110 shadow-md' : 'bg-subtle text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700'}`}
            title={habit.title}
          >
            <i className={`fa-solid ${habit.icon || 'fa-check'}`}></i>
          </button>
        ))}
      </div>
    </div>
...
export const CourseProgressWidget: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-full bg-surface p-4 justify-center space-y-3">
      {courses.length === 0 && <div className="text-xs text-gray-400 italic text-center">No courses tracked</div>}
      {courses.map(course => (
        <div key={course.code}>
          <div className="flex justify-between text-xs mb-1">
            <span className="font-bold text-ink">{course.code}</span>
            <span className="text-gray-500">{course.progressPercent}%</span>
          </div>
          <div className="h-1.5 bg-subtle rounded-full overflow-hidden">
            <div
              className={`h-full rounded-full ${course.progressPercent >= 100 ? 'bg-green-500' : 'bg-blue-500'}`}
...
export const RecentFilesWidget: React.FC = () => {
  ...
  const getIcon = (type: string) => {
    switch (type.toLowerCase()) {
      case 'note': return 'fa-note-sticky text-yellow-500';
      case 'project': return 'fa-folder-open text-blue-500';
      case 'deck': return 'fa-layer-group text-purple-500';
  ...
  return (
    <div className="flex flex-col h-full bg-surface p-4">
      ...
      <div className="space-y-2 overflow-y-auto custom-scrollbar flex-1">
        {files.length === 0 && <div className="text-xs text-gray-400 italic">No recent files</div>}
        {files.map((file, i) => (
          <div key={i} className="flex items-center gap-2 text-sm text-ink hover:text-blue-500 cursor-pointer transition-colors max-w-full">
            <i className={`fa-solid ${getIcon(file.type)}`}></i>
